...
`);
    ...
    await progress("RENDERING NETWORK TOPOLOGY. (Try)");
 ...
 print(text);
  ...
  try{
    ...
    typePrint(`
Downloading recovered audio log...
...
`)
 ...
 window.open("https://cloudzilla-nmit.vercel.app/");
  ...
  window.open("https://i.ytimg.com/vi/8p-hs-5tKmM/sddefault.jpg"); 
}
...
window.open("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSommEqfnspEYVrNdx1aGAoeMn7X92NCzYCnA&s");
 ...
 await glitch("[AI] "+hints[Math.floor(Math.random()*hints.length)]);
    ...
    if(data.correct){
  triggerSuccess();
  await typePrint("✅ CLUE VERIFIED (+25 points)");
}else{
  triggerError();
  triggerShake();
...
function launchEndSequence(){
  ...
  const timerEl = document.getElementById("eventTimer");
  if(timerEl){
    timerEl.textContent = "🏆 INVESTIGATION COMPLETE";
  }
  ...
  document.getElementById("creditTeamName").textContent =
    "Team: " + team;
